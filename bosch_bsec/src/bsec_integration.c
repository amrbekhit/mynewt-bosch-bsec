...
static struct sensor *bme680_sensor;

/* Global sensor data struct */
static struct bme680_field_data SensorData;
 ...
 */
int bsec_iot_init(float sample_rate, float temperature_offset, struct sensor *bme680, sleep_fct sleep, state_load_fct state_load, config_load_fct config_load)
{
    int ret = 0;
    ...
    /* Store the pointer to the sensor device */
    ...
    {       
        ret = bsec_set_configuration(bsec_config, bsec_config_len, work_buffer, sizeof(work_buffer));     
        if (ret != BSEC_OK)
        {
            return ret;
        }
    ...
    if (bsec_state_len != 0)
    {       
        ret = bsec_set_state(bsec_state, bsec_state_len, work_buffer, sizeof(work_buffer));     
        if (ret != BSEC_OK)
        {
            return ret;
        }
    ...
    ret = bme680_bsec_update_subscription(sample_rate);
    if (ret != BSEC_OK)
    {
        return ret;
    }
...
static int bme680_sensor_read_cb(struct sensor *sensor, void *arg, void *data, sensor_type_t type) 
{
    int rc = 0;
    struct bme680_field_data *sensor_data = arg;

    switch (type) {
        case SENSOR_TYPE_TEMPERATURE:
            memcpy(&sensor_data->temperature, data, sizeof(sensor_data->temperature));
            break;
        case SENSOR_TYPE_PRESSURE:
            memcpy(&sensor_data->pressure, data, sizeof(sensor_data->pressure));
            break;
        case SENSOR_TYPE_RELATIVE_HUMIDITY:
            memcpy(&sensor_data->humidity, data, sizeof(sensor_data->humidity));
            break;
        case SENSOR_TYPE_USER_DEFINED_1:
            memcpy(&sensor_data->gas_resistance, data, sizeof(sensor_data->gas_resistance));
            sensor_data->status |= BME680_GASM_VALID_MSK;
            break;
        default:
            rc = 1;
            break;
...
static void bme680_bsec_trigger_measurement(bsec_bme_settings_t *sensor_settings, sleep_fct sleep)
{
    ...
    {
        //NRF_TWIM0->ENABLE = TWI_ENABLE_ENABLE_Disabled; // TWI_ENABLE_ENABLE_Enabled;
        sensor_read(bme680_sensor, SENSOR_TYPE_ALL, bme680_sensor_read_cb, &SensorData, OS_TICKS_PER_SEC);
    ...
    int32_t bsec_process_data)
{
    ...
    if (bsec_process_data)
    {
        ...
        if (bsec_process_data & BSEC_PROCESS_PRESSURE)
        {
            ...
            inputs[*num_bsec_inputs].signal = SensorData.pressure;
        ...
        if (bsec_process_data & BSEC_PROCESS_TEMPERATURE)
        {
            ...
            inputs[*num_bsec_inputs].signal = SensorData.temperature;
#else
            inputs[*num_bsec_inputs].signal = SensorData.temperature / 100.0f;
#endif
            inputs[*num_bsec_inputs].time_stamp = time_stamp_trigger;
            (*num_bsec_inputs)++;
...
#if MYNEWT_VAL(BME680_FLOATING_POINT_COMPENSATION)
            inputs[*num_bsec_inputs].signal = SensorData.humidity;
#else
            inputs[*num_bsec_inputs].signal = SensorData.humidity / 1000.0f;
#endif
            inputs[*num_bsec_inputs].time_stamp = time_stamp_trigger;
            (*num_bsec_inputs)++;
        }
        ...
        if (bsec_process_data & BSEC_PROCESS_GAS)
        {
            ...
            if(SensorData.status & BME680_GASM_VALID_MSK)
            {
                /* Place sample into input struct */
                inputs[*num_bsec_inputs].sensor_id = BSEC_INPUT_GASRESISTOR;
                inputs[*num_bsec_inputs].signal = SensorData.gas_resistance;
                inputs[*num_bsec_inputs].time_stamp = time_stamp_trigger;
                (*num_bsec_inputs)++;
            }
        }
